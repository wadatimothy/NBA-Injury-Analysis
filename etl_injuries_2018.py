...
from pyspark.sql.functions import (
    col, trim, upper, lower, when, regexp_extract, regexp_replace, coalesce,
    to_date, year, month, dayofweek, lpad, concat_ws, sha2, lit
)
...
def derive_injury_region_and_severity(df):
    ...
    df = df.withColumn(
        "injury_severity",
        when(text.rlike(r"\b(surgery|repair|fracture|broken|rupture|torn|tear)\b"), "HIGH")
        .when(text.rlike(r"\b(grade\s*2|grade\s*iii|partial tear|dislocation)\b"), "MEDIUM")
        .when(text.rlike(r"\b(sprain|strain|soreness|contusion|bruise|tightness)\b"), "LOW")
        .otherwise("UNKNOWN")
    )
...
def build_dims_and_fact(df):
    ...
    dim_player = df18.select("Player_Name").dropDuplicates() \
        .withColumn("player_key", sha2(upper(col("Player_Name")), 256))
    dim_team = df18.select("Team").dropDuplicates() \
        .withColumn("team_key", sha2(col("Team"), 256))

    dim_injury = df18.select("injury_category", "injury_region", "injury_severity") \
        .fillna({"injury_category":"OTHER","injury_region":"OTHER","injury_severity":"UNKNOWN"}) \
        .dropDuplicates() \
        .withColumn("injury_key", sha2(concat_ws("|", col("injury_category"), col("injury_region"), col("injury_severity")), 256))

    # Date dimension: YYYYMMDD key
    dim_date = df18.select("Date").dropDuplicates() \
    ...
    # Join to create fact
    fact = df18.join(dim_player, "Player_Name", "left") \
               .join(dim_team, "Team", "left") \
               .join(dim_injury, ["injury_category","injury_region","injury_severity"], "left") \
               .join(dim_date, "Date", "left") \
               .withColumn("fact_injury_key",
                           sha2(concat_ws("|",
                                col("player_key"), col("team_key"), col("injury_key"),
                                col("date_key").cast(StringType()), coalesce(col("Notes"), lit(""))
                           ), 256)) \
               .select(
                   "fact_injury_key",
                   "player_key", "team_key", "injury_key", "date_key",
                   "Player_Name", "Team",
                   "injury_category", "injury_region", "injury_severity",
                   "Injury_Type", "Notes"
               )
...
def main():
    ...
    p.add_argument("--raw-injuries", required=True, help="s3a path to raw injuries CSV(s)")
    p.add_argument("--clean-prefix", required=True, help="s3a prefix for clean outputs, e.g. s3a://bucket/clean/nba/")
    ...
    write_parquet(dim_injury.select("injury_key","injury_category","injury_region","injury_severity"), f"{base}/dim_injury")
